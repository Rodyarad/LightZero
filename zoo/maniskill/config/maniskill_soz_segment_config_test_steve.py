...
def main(seed, model_path=None):
    ...
    checkpoint_path = 'zoo/ocr/steve_weights/steve_maniskill.pt'
    ...
    create_config = EasyDict(
        dict(
            env=dict(type='maniskill_lightzero', import_names=['zoo.maniskill.env.maniskill_lightzero_env']),
            env_manager=dict(type='base'),
            policy=dict(type='sampled_unizero', import_names=['lzero.policy.sampled_unizero']),
        )
    )

    from lzero.entry import train_unizero_segment
    main_config.exp_name = (
        f'data_sampled_unizero/maniskill_Push_steve_brf{buffer_reanalyze_freq}'
        f'_image_cont_suz_nlayer{num_layers}_numsegments-{num_segments}_gsl{game_segment_length}'
        f'_K{K}_ns{num_simulations}_rr{replay_ratio}_Htrain{num_unroll_steps}'
        f'-Hinfer{infer_context_length}_bs{batch_size}_{norm_type}_seed{seed}_learnsigma'
    )
