...
from lzero.policy import scalar_transform, cross_entropy_loss, phi_transform, \
    ...
    mz_rnn_fullobs_network_output_unpack, negative_cosine_similarity, mse_loss, \
    prepare_obs
from lzero.policy.muzero import MuZeroPolicy
from lzero.entry.utils import initialize_zeros_batch


@POLICY_REGISTRY.register('objectzero_gru')
class ObjectZeroGRUPolicy(MuZeroPolicy):
    """
    Overview:
    ...
    config = dict(
        model=dict(
            ...
            model_type='slot',  # options={'mlp', 'conv', 'slot'}
            # (bool) If True, the action space of the environment is continuous, otherwise discrete.
            continuous_action_space=False,
            # (tuple) The stacked obs shape.
            observation_shape=(6, 64),
    ...
    def default_model(self) -> Tuple[str, List[str]]:
        ...
        if self._cfg.model.model_type == "slot":
            return 'ObjectZeroGRUModel', ['lzero.model.objectzero_gru_model']
        else:
    ...
    def _forward_learn(self, data: torch.Tensor) -> Dict[str, Union[float, int]]:
        ...
        for step_k in range(self._cfg.num_unroll_steps):
            ...
            if self._cfg.model.model_type == 'slot':
                next_obs_batch = next_obs_batch.squeeze(1)

            next_latent_state = to_tensor(next_obs_batch)
            ...
            if self._cfg.ssl_loss_weight > 0:
                ...
                #temp_loss = negative_cosine_similarity(predict_next_latent_state, next_latent_state) * mask_batch[:, step_k]
                temp_loss = mse_loss(predict_next_latent_state, next_latent_state) * mask_batch[:, step_k]
    ...
    def _init_eval(self) -> None:
        ...
        if self._cfg.model.model_type == 'conv':
            self.last_batch_obs = torch.zeros([self._cfg.evaluator_env_num, self._cfg.model.observation_shape[0], 64, 64]).to(self._cfg.device)
            self.last_batch_action = [-1 for i in range(self._cfg.evaluator_env_num)]
        elif self._cfg.model.model_type == 'mlp':
            self.last_batch_obs = torch.zeros([self._cfg.evaluator_env_num, self._cfg.model.observation_shape]).to(self._cfg.device)
            self.last_batch_action = [-1 for i in range(self._cfg.evaluator_env_num)]
        elif self._cfg.model.model_type == 'slot':
            self.last_batch_obs = torch.zeros([self._cfg.evaluator_env_num, *self._cfg.model.observation_shape]).to(self._cfg.device)
