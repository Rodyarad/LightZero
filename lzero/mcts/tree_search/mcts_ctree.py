...
class UniZeroActionAbstractionMCTSCtree(object):
    """
    Overview:
        MCTSCtree for UniZero. The core ``batch_traverse`` and ``batch_backpropagate`` function is implemented in C++.
    ...
    def __init__(self, cfg: EasyDict = None) -> None:
        ...
        self.value_inverse_scalar_transform_handle = InverseScalarTransform(self.value_support, self._cfg.model.categorical_distribution)
        self.reward_inverse_scalar_transform_handle = InverseScalarTransform(self.reward_support, self._cfg.model.categorical_distribution)
        # Object-level masking configuration.
        self._num_objects = self._cfg.model.num_objects
        self._action_space_size = self._cfg.model.action_space_size
        # Default: split actions into contiguous groups per object.
        assert self._num_objects is not None and self._num_objects > 0, \
            "num_objects must be set"
        actions_per_obj = self._action_space_size // self._num_objects
        assert actions_per_obj * self._num_objects == self._action_space_size, \
            "action_space_size must be divisible by num_objects for default grouping"
    ...
    def search(
        ...
        with torch.no_grad():
            ...
            for simulation_index in range(self._cfg.num_simulations):
                ...
                # Optional per-node object-level action masking via dedicated mask head (if present).
                leaf_legal_actions_list = None
                # Respect causal_puct_start_step: before this train_iter, do not use model-based masks in the tree.
                train_iter = self.train_iter
                causal_start = self._cfg.causal_puct_start_step
                mask_logits = network_output.mask_logits
                if (
                    train_iter >= causal_start
                    and mask_logits is not None
                    and self._num_objects is not None
                ):
                    mcts_mask_thres = self._cfg.model.mcts_mask_thres
                    if mcts_mask_thres is not None:
                        # Object mask: shape (batch_size, num_objects)
                        obj_mask = (torch.sigmoid(mask_logits) > mcts_mask_thres)
                        obj_mask = obj_mask.detach().cpu().numpy().astype(bool)
                        leaf_legal_actions_list = []
                        for i in range(batch_size):
                            # Map object mask -> action mask using obj_of_action mapping.
                            per_obj = obj_mask[i]  # (num_objects,)
                            action_mask = per_obj[self._obj_of_action]  # (action_space_size,)
                            legal = np.where(action_mask)[0].tolist()
                            # Fallback: if everything is masked out, ignore mask for this leaf.
                            if len(legal) == 0:
                                legal = list(range(self._action_space_size))
                            leaf_legal_actions_list.append(legal)
                ...
                # NOTE: simulation_index + 1 is very important, which is the depth of the current leaf node.
                current_latent_state_index = simulation_index + 1
                if leaf_legal_actions_list is not None and hasattr(tree_muzero, 'batch_backpropagate_with_legal_actions'):
                    tree_muzero.batch_backpropagate_with_legal_actions(
                        current_latent_state_index, discount_factor, reward_batch, value_batch, policy_logits_batch,
                        min_max_stats_lst, results, virtual_to_play_batch, leaf_legal_actions_list
                    )
                else:
                    tree_muzero.batch_backpropagate(
                        current_latent_state_index, discount_factor, reward_batch, value_batch, policy_logits_batch,
