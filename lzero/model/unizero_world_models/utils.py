...
def apply_object_mask_to_policy_logits_with_gumbel(
    ...
    Overview:
        ...
        - Mask policy logits by adding a large negative value to invalid actions.
    ...
    Arguments:
        ...
        - eps (:obj:`float`): Numerical stability epsilon.
        - mask_invalid_value (:obj:`float`): Large (positive) magnitude; invalid actions get
          \(-(\\text{mask\\_invalid\\_value})\) added to their logits.
    ...
    """
    if mask_logits_obj is None:
        raise ValueError("apply_object_mask_to_policy_logits_with_gumbel: mask_logits_obj must not be None.")

    # Gumbel-Sigmoid over objects
    mask_soft = gumbel_sigmoid(mask_logits_obj, temp=mask_temp, eps=eps)  # (B, T, num_objects)

    # Straight-through hard threshold
    mask_hard = (mask_soft > mask_thres).float()
    mask_obj = straight_through_estimator(mask_soft, mask_hard)  # (B, T, num_objects)
    ...
    # Add large negative value to masked-out actions
    masked_logits = logits_policy + (mask_action - 1.0) * mask_invalid_value
    masked_logits = logits_policy * mask_action + (mask_action - 1.0) * mask_invalid_value
